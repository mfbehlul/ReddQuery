    ...
    for item in data:
        ...
        try:
            for top_level_comment,polarity_value,subjectivity_value in zip(item["comment"],item["polarity"],item["subjectivity"]):
                ...
                cur.execute("INSERT INTO results_results(title,postid,score,author,subreddit,comment,polarity,subjectivity,time)values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                            (item["post"], item["post_id"], item["score"], item["author"], item["subreddit"], top_level_comment, polarity_value,subjectivity_value,"nodata"))
